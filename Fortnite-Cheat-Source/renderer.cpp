...
namespace rend {
	...
	VOID EndScene(ImGuiWindow& window)
	{
		...
		if (showMenu)
		{
			if (ImGui::Begin(xorstr("PlutoniumFN"), 0, ImGuiWindowFlags_::ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_::ImGuiWindowFlags_NoResize))
			{
				ImGui::Separator();

				if (ImGui::Button("Discord"))
				{
					system("start https://dsc.gg/pluto69");
				}

				ImGui::Separator();

				ImGui::Text("Created By Plutonium#9200");
				...
				if (ImGui::CollapsingHeader("Player ESP"))
				{
					...
					ImGui::Checkbox("SnapLines ESP", &Settings.ESP.PlayerLines);
